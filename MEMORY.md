...
- **결과**: 6개 항목 모두 OK, CRITICAL 없음. `.env.local`은 git에 한 번도 커밋된 적 없고,
  `NEXT_PUBLIC_*`엔 서버 전용 키가 없다. `SUPABASE_SERVICE_ROLE_KEY`·`GEMINI_API_KEY`는
  `lib/supabase-admin.ts`와 `app/api/extract-project/route.ts`처럼 서버 라우트에서만 쓰인다.
  `projects`·`stages`·`tasks` 테이블 모두 RLS가 켜져 있고 정책을 두지 않아 anon은 완전히 차단,
  service_role만 우회 가능 — 처음부터 "로그인 없음 + 비공개 데이터" 패턴대로 만들어서 점검에서
  새로 고칠 게 없었다. 민감정보 검사에서 "card"로 매치된 건 전부 shadcn `Card` UI 컴포넌트
  이름이었고(오탐), 실제 비밀번호·카드·계좌·주민번호 컬럼은 스키마에 존재하지 않는다.
- **남은 위험**: 없음 — 다만 이 점검은 이름 기반 휴리스틱이라 나중에 민감정보 컬럼을 추가하면
  다시 점검해야 한다.
- **배운 것**: 처음 설계 때 RLS·서버 라우트 분리 원칙을 지켜두면, 나중에 보안 점검을 돌렸을 때
  고칠 게 없어 점검 자체가 훨씬 빨리 끝난다.
